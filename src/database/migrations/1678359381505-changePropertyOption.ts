...
export class changePropertyOption1678359381505 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE TABLE \`option_candidates\`
                             (
    ...
    await queryRunner.query(`ALTER TABLE \`property_options\`
        ADD \`option_id\` int NULL`);
    await queryRunner.query(`ALTER TABLE \`option_candidates\`
        ADD CONSTRAINT \`FK_46cfb0bda78e7f6c82f664f5612\` FOREIGN KEY (\`property_id\`) REFERENCES \`properties\` (\`id\`) ON DELETE CASCADE ON UPDATE RESTRICT`);
    await queryRunner.query(`ALTER TABLE \`property_options\`
        ADD CONSTRAINT \`FK_2244344c11f1195f41745a8fe8e\` FOREIGN KEY (\`option_id\`) REFERENCES \`option_candidates\` (\`id\`) ON DELETE CASCADE ON UPDATE RESTRICT`);
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE \`property_options\` DROP COLUMN \`option_id\``);
    await queryRunner.query(`ALTER TABLE \`property_options\`
        ADD \`option_id\` varchar(255) NOT NULL`);
    await queryRunner.query(`ALTER TABLE \`property_options\`
