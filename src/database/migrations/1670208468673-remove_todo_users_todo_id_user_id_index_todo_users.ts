...
export class removeTodoUsersTodoIdUserIdIndexTodoUsers1670208468673 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE todo_users ADD FOREIGN KEY (todo_id) REFERENCES todos (id)  ON DELETE SET NULL`
    );

    await queryRunner.query(
      `ALTER TABLE todo_users ADD FOREIGN KEY (user_id) REFERENCES users (id)  ON DELETE SET NULL`
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
        ALTER TABLE todo_users ADD
